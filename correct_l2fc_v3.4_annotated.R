...
#' a larger bin size (200,1000) can be used, as there are more growth-deficient strains. By default, window size is 1% of total strain size. This is a good default. 
#' 
...
correct_l2fc_v3 <- function(initial_gammas, other_gammas, window_number = NA, norm_to_constant = NA){
  ...
  #default bin size (1%), if not specified
  if (is.na(window_number)){
    
    #initial guess
    temp <- as.integer(length(not_na)/100)
    temp2 <- temp+1
    
    #if the bins are perfect use it, else
    if (length(not_na) %% temp == 0){window_number <- temp} 
    if (length(not_na) %% temp2 == 0){window_number <- temp2}
    
    #if neither possible bin is perfect
    if (length(not_na) %% temp != 0 &
        length(not_na) %% temp2 != 0){
      
      #pick the one with the largest modulus
      if ((length(not_na) %% temp) >= (length(not_na) %% temp2)){
        window_number <- temp
      } else {
        window_number <- temp2
      }
